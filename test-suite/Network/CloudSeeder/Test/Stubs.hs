...
instance (AsArgumentsError e, MonadError e m) => MonadArguments e (ArgumentsT m) where
...
newtype LoggerT m a = LoggerT (WriterT [ByteString] m a)
  deriving ( Functor, Applicative, Monad, MonadTrans, MonadError e
           , MonadArguments e, MonadFileSystem e, MonadCloud, MonadEnvironment )
...
newtype FileSystemT m a = FileSystemT (ReaderT [(T.Text, T.Text)] m a)
  deriving ( Functor, Applicative, Monad, MonadTrans, MonadError e
           , MonadArguments e, MonadLogger, MonadCloud, MonadEnvironment )
...
newtype EnvironmentT m a = EnvironmentT (ReaderT (M.Map T.Text T.Text) m a)
  deriving ( Functor, Applicative, Monad, MonadTrans, MonadError e
           , MonadArguments e, MonadLogger, MonadFileSystem e, MonadCloud )
